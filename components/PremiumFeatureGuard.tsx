...
export function PremiumFeatureGuard({
  ...
  if (loading) {
    return (
      <div className="rounded-xl border border-border bg-[var(--surface-soft)] p-4 text-sm text-foreground/50">
        Loading access…
      </div>
    );
  }
  ...
  if (!enabled) {
    return (
      <div className="rounded-xl border border-dashed border-border bg-[var(--surface-soft)] p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-3 text-xs">
          <Link
            href="/pricing"
            className="inline-flex items-center rounded-full bg-emerald-500 px-4 py-1.5 font-semibold text-foreground hover:bg-emerald-400"
          >
            View plans
          </Link>
          <Link
            href="/demo"
            className="inline-flex items-center rounded-full border border-border px-4 py-1.5 text-foreground hover:bg-card-2"
          >
            Book a walkthrough
          </Link>
        </div>
